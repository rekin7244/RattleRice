...
public class SearchFreeBoardServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		limit = 5;
		int listCount = new BoardService().getSearchFreeBoardListCount(keyword);
		maxPage = (int)((double)listCount / limit + 0.9);
		startPage = (currentPage - 1) * limit + 1;
		endPage = startPage + limit - 1;
		
		PageInfo pi = new PageInfo(currentPage,limit,maxPage,startPage,endPage);
		ArrayList<Board> list = new BoardService().searchFreeBoardList(pi, keyword);
